...
var text_dien_DA = document.getElementById("diendapan");
var option_so_motDA = document.getElementById("Option_motDA");
var option_so_nhieuDA = option_so_nhieuDA;
...
option_so_nhieuDA;.addEventListener('input', function(){
...
option_so_motDA.addEventListener('input', function(){
    var Option = parseInt(this.value);
...
function collect() {
    ...
    if(select_loai_DA.value === 'Option_dienDA'){
        var fill = text_dien_DA.value.trim();
        if(fill !== ""){
...
function Kiem_tra_checked() {
    if(select_loai_DA.value === 'Option_dienDA'){
        const fill = text_dien_DA.value.trim();
        if(fill === ""){
    ...
    if(danh_sach_cau_hoi[index].Trangthai === "Đã duyệt" || danh_sach_cau_hoi[index].Trangthai === "Không được duyệt" ){
        ...
        if(confirm("Bạn muốn sửa câu hỏi ?")){
            ...
            if(question.LoaiCauhoi =="Điền đáp án") {
                ...
                text_dien_DA.value = question.Answer;
